...
static sbool dynstats_shouldPersist(dynstats_bucket_t *b, time_t now) {
	...
	pthread_rwlock_rdlock(&b->lock);
	shouldPersist = ( (b->persist_state_write_count_interval > 0 && ++b->n_updates >= b->persist_state_write_count_interval) ||
...
enqueueFileWriteTask(dynstats_bucket_t *b, const char* file_name, const char* content) {
	...
	bkts->work_q.size++;
	STATSCOUNTER_INC(bkts->work_q.ctrEnq, work_q.mutCtrEnq);
