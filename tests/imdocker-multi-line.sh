...
docker run \
   ...
   alpine \
   /bin/sh -c \
   'for i in `seq 1 $num_items`; do printf "multi-line: item $i\n  line2....\n line3....\n"; sleep .01; done' > /dev/null

NUM_EXPECTED=$(($NUM_ITEMS-1))
echo "expected: $NUM_EXPECTED"

content_check_with_count "HEADER multi-line:" $NUM_EXPECTED
## check if all the data we expect to get in the file is there
for i in `seq 1 $NUM_EXPECTED`; do
  grep "HEADER multi-line: item $i" $RSYSLOG_OUT_LOG > /dev/null 2>&1
  if [ ! $? -eq 0 ]; then
    echo "ERROR: expecting the string HEADER multi-line: item '$i', it's not there"
    exit 1
  fi
done
